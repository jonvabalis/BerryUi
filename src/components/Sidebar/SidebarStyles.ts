...
// Children Component
export const Children = styled.div<SidebarProps>`
  width: 100%;
  height: 100%;
...
export const SidebarLogoWrapper = styled.div<SidebarProps>`
  padding: 0.5rem 1rem;
  margin-bottom: 1rem;
  display: flex;
...
export const SidebarBrand = styled.span<SidebarProps>`
  display: ${({ displaySidebar }) => (displaySidebar ? "block" : "none")};
`;

export const SidebarToggler = styled.button<SidebarProps>`
  cursor: pointer;
  display: ${({ displaySidebar }) => (displaySidebar ? "block" : "none")};
  @media (max-width: 468px) {
    display: block;
...
export const ItemName = styled.span<SidebarProps>`
